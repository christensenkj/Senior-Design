...
unsigned char check_Response(){
    ...
    TA0CCR0 = 25;        //Set timer to overflow in 100uS. 100 @ 1x divider
    SET(TA0CCTL0,CCIE);  //And enable timer interrupt
    ...
    if (TOUT)
        ...
        if(TOUT)
            ...
            CLR(TA0CCTL0,CCIE);  // Disable timer interrupt
            return 1;
        }
    }
}
